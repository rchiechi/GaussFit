...
class PreferencesWindow(Toplevel):
    def __init__(self, parent, opts_parser, opts):
        ...
        # Pass the parser instance to the frame
        self.prefs = PreferencesFrame(self, opts_parser, opts)
        self.transient(parent)
        self.grab_set()

class PreferencesFrame(ttk.Frame):
    def __init__(self, master, opts_parser, opts):
        ...
        # A map to direct argument groups to the correct pane.
        # The keys MUST EXACTLY match the group titles from args.py.
        self.pane_map = {
    ...
    def __create_buttons(self):
        ...
        save_button = ttk.Button(button_frame, text="Save and Close", command=self.master.destroy)
        save_button.pack()

    def __create_dynamic_options(self):
        """
        Dynamically generate options by introspecting the argparse opts_parser.
        This replaces the original __createOptions and all check... methods.
        """
        for group in self.opts_parser._action_groups:
            # Use the pane_map to find where to place this group's frame
            ...
            for action in group._group_actions:
                # Skip the built-in help action and any "negative" boolean flags
    ...
    def update_list_opt(self, dest, var, type_func):
        ...
        value_str = var.get()
        try:
            # Split string, strip whitespace, and convert to target type
